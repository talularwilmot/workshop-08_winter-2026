...
drought_exp_clean <- drought_x %>% 
  # celaning the names of the rows to remove spaces and capital 
  clean_names() %>% 
  # adding a column called species_name change the names of plant species to be full name 
  ...
  # adding a column called water_treatment and changing the names of the treatment groups to full names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) %>% 
  # making sure categorical variables are treated as factors and setting level order 
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) %>% 
  # select only these column names to be included in the data frame 
...
labs( x = "Water treatment", 
        y = "Total mass (g)", 
        title = "Mean weight of well watered treatment group greater than drought stressed") + 
